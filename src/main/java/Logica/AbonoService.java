...
public class AbonoService {
    ...
    public PrepagoResponseDTO obtenerPrepago(String cliente) {
        try {
            ...
            URL url = new URL(baseUrl + "/api/escritorio/abonos/prepago/" + cliente);
    ...
    public DefaultTableModel mostrar(String buscar) throws Exception {
        ...
        for (Dabonosp ab : lista) {
            Object[] registro = new Object[]{
                ...
                ab.getTarjeta(),
                ab.getTransferencia(),
                ab.getTotal(), // ✅ Ahora total va aquí
                ab.getSaldo(), // ✅ Y saldo va después
                ab.getTurno(),
                ab.getEmpleado(),
                ab.getObservaciones(),
                ab.getNumeroturno(),};
    ...
    public void llenarcboClientes(JComboBox<String> combo) {
        ...
        try {
            ...
            if (status != 200) {
                System.err.println("Error al consultar clientes. Código: " + status);
    ...
    public boolean insertar(AbonoDTO abono) {
        try {
            ...
            String baseUrl = ConfiguradorApi.getInstance().getApiBaseUrl();
            URL url = new URL(baseUrl + "/api/escritorio/abonos");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    ...
    public boolean editar(Integer id, AbonoDTO abono) {
        try {

            String baseUrl = ConfiguradorApi.getInstance().getApiBaseUrl();
            URL url = new URL(baseUrl + "/api/escritorio/abonos/" + id);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
